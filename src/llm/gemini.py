...
import os
import yaml
from utils.gemini_schema import gemini_schema
...
model = runs_config["llm"]["gemini"]["model"]
...
classifier.save_results(
    zero_shot_results, runs_config["llm"]["gemini"]["output_dir"], "zero_shot"
)
...
classifier.save_results(
    few_shot_results, runs_config["llm"]["gemini"]["output_dir"], "few_shot"
)

print("Gemini inference completed.")

# --- Evaluate performance on test dataset ---
# Directories for saving outputs
output_dir = runs_config["llm"]["gemini"]["output_dir"]
...
for run_name, y_pred in zip(run_names, [y_pred_zero_shot, y_pred_few_shot]):
    ...
    cm_path = os.path.join(
        runs_config["llm"]["gemini"]["output_dir"],
        f"gemini_{run_name}_confusion_matrix_{timestamp}.png",
    )
    ...
    metrics_path = os.path.join(
        runs_config["llm"]["gemini"]["output_dir"],
        f"gemini_{run_name}_performance_metrics_{timestamp}.txt",
    )
...
print("Evaluation of Gemini inference completed.")
